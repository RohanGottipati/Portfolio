  ...
  return (
    <article ref={articleRef} className="mx-auto max-w-[960px] px-5 py-12 md:px-10 md:py-16">
      ...
      {project.caseStudy && <>
        ...
        {project.caseStudy.metrics && <section className={`mt-14 grid gap-4 border-y border-ink/20 py-7 text-center ${project.caseStudy.metrics.length > 1 ? 'sm:grid-cols-2' : 'grid-cols-1'}`}>{project.caseStudy.metrics.map(metric => <div key={metric.label}><strong className="font-display text-5xl font-normal text-tangerine">{metric.value}</strong><p className="mt-1 font-mono text-[10px] uppercase tracking-widest text-ink-soft">{metric.label}</p></div>)}</section>}
      </>}
